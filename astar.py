"""
Gamified Astar implementation with pygame
...
class Cell():
    ...
    def draw(self):
        # uncomment different costs below to see different point systems
        COST_TO_DRAW = ''
...
class Paul(Actor):
    ...
    def get_open_adj_coords(self, coords):
        ...
        for i, coord in enumerate(all_hops):
            if (in_bounds3[i]):
                costs.append(6 + self.world.get_terrain_cost(coord))
                open_adj.append(coord)
